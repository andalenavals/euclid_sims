...
ADAMOMDIR=$WORKDIR/adamom_sexcat_nw_ss_sub2_biguesig_fixstamp
...
python run_sim_meas_constimg_euclidlike.py --loglevel=INFO --transformtogriddir=$BLENDEDDIR --simdir=$SIMDIR --sexdir=$SEXDIR --adamomdir=$ADAMOMDIR --ksbdir=$KSBDIR --adamompsfcatalog=$ADAMOMPSFCAT --groupcats=$GROUPCATS --cat_args=$CAT_ARGS --sex_args=$SEX_ARGS --constants=$CONSTANTS --tru_type=2 --pixel_conv --dist_type=flagship --cattype=sex --ncpu=220 --ncat=2000 --typegroup=tw --skipdone --substractsky --subsample_nbins=2 --runadamom --matchinput #--runsex #--runsims --usepsfimg --usevarpsf --usevarsky --runsex #--run_check #--use_weight  #--transformtogrid --runsims --usepsfimg --usevarpsf --usevarsky --runsex #--run_check #--use_weight 
