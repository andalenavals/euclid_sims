...
WORKDIR=/vol/euclidraid5/data/aanavarroa/catalogs/MomentsML2/fullimages_constimg_euclid-flagship0.05/tw-test-grid-cosmos
SIMDIR=$WORKDIR/sim
...
python run_sim_meas_constimg_euclidlike.py --loglevel=DEBUG --selected_flagshipcat=$SELECTED_FLAGSHIP --cosmoscatfile=$COSMOSCAT --cosmosdir=$COSMOSDIR --simdir=$SIMDIR --sexdir=$SEXDIR --adamomdir=$ADAMOMDIR --ksbdir=$KSBDIR --adamompsfcatalog=$ADAMOMPSFCAT --groupcats=$GROUPCATS --cat_args=$CAT_ARGS --sex_args=$SEX_ARGS --constants=$CONSTANTS --tru_type=3 --pixel_conv --substractsky --subsample_nbins=2 --dist_type=flagship --cattype=tru --ncpu=10 --ncat=10 --skipdone --typegroup=tw --runksb --runsex --runsims --usepsfimg --rot_pair --usevarpsf #--runksb #--runadamom  #--runsex #--usevarsky #--matchinput #--runsex #--adamom_weight  #--runsex #--runksb 
