...
python run_sim_meas_constimg_euclidlike.py --loglevel=DEBUG --simdir=$SIMDIR --sexdir=$SEXDIR --adamomdir=$ADAMOMDIR --ksbdir=$KSBDIR --adamompsfcatalog=$ADAMOMPSFCAT --groupcats=$GROUPCATS --cat_args=$CAT_ARGS --sex_args=$SEX_ARGS --constants=$CONSTANTS --tru_type=2 --pixel_conv --substractsky --dist_type=uniflagship --cattype=tru --ncpu=200 --ncat=5000 --skipdone --typegroup=tp --runadamom --match_pairs #--runsex #--runsims --usepsfimg --usevarpsf --usevarsky #--runsex #--usevarsky #--matchinput #--runsex #--adamom_weight  #--runsex #--runksb 
