...
ADAMOMDIR=$WORKDIR/adamom_sexcat_w_ss1.0_sub2_biguesig_fixestamp
...
python run_sim_meas_constimg_euclidlike.py --loglevel=INFO --simdir=$SIMDIR --sexdir=$SEXDIR --adamomdir=$ADAMOMDIR  --ksbdir=$KSBDIR --adamompsfcatalog=$ADAMOMPSFCAT --groupcats=$GROUPCATS --cat_args=$CAT_ARGS --sex_args=$SEX_ARGS --constants=$CONSTANTS --tru_type=2 --pixel_conv --substractsky --dist_type=flagship --cattype=sex --ncpu=100 --ncat=2000 --typegroup=tw --skipdone --nsubcases=1 --subsample_nbins=2 --runadamom --matchinput --use_weight # --add_ics --runsex --run_check #--runsims --usepsfimg --usevarsky --usevarpsf #--run_check  #--runsex --run_check --substractsky 
