...
WORKDIR=/vol/euclidraid5/data/aanavarroa/catalogs/MomentsML2/fullimages_constimg_euclid-flagship0.05/truesize-varpsf-varback-grid
...
python run_sim_meas_constimg_euclidlike.py --loglevel=INFO --simdir=$SIMDIR --sexdir=$SEXDIR --adamomdir=$ADAMOMDIR --ksbdir=$KSBDIR --adamompsfcatalog=$ADAMOMPSFCAT --groupcats=$GROUPCATS --cat_args=$CAT_ARGS --sex_args=$SEX_ARGS --constants=$CONSTANTS --tru_type=2  --pixel_conv --substractsky --dist_type=uniflagship --cattype=tru --ncpu=60 --ncat=5000 --skipdone --typegroup=tp --subsample_nbins=2 --runadamom #--max_shear=0 --usevarpsf --runsims --usepsfimg --usevarsky #--runsex --usevarpsf #--adamom_weight --runsex 
