...
def main():
    ...
    args = Arguments(        
        max_iterations=10,
        mag_zeropoint=32.19, 
        image_file="sample.fits",
        psf_file="samplepsf.fits"
    )
    ...
    if args.psf_file is not None:
        top = load_fits_image(args.image_file,args.psf_file)
    else:
        top = load_fits_image(args.image_file)
    ...
    # For each band (group)
    #for group in measurement_group:
    # Flux is fitted once per group
    
    #print("MR MEESSEEKS \n",  type(group))
    #print(args.force_stop)
    ...
    add_output_column(f'mf_flux', flux)
    add_output_column(f'mf_mag', mag)
